...
Fuzz testing - один из видов тестирования, который помогает выявить краевые некорректные случаи в работе программы. Работа данного вида заключается не в обработке конкретных случаев, а полное нагрузочное тестирование с непредсказуемыми случаями, но с предсказуемым результатом. Тем самым мы выявляем ошибки и находим самые узкие места в нашей программе.

Для тестирования я взял один из модулей из занятия по АСД_1 - Linked List, где вышел он на ~200 строк кода, ЯП - Go.
...
- если рассматривать платформу Go, то нативные инструменты поддерживают не все архитектуры процессоров
- использование Fuzzing тестирования на других языках пожразумевает установку дополнительных зависимостей, что не совсем удобно
- узкая поддержка типов данных, так как такой тип тестирования тесно связан с алгоритмами псевдослучайной генерации, а она совместима не со всеми типами данных, например, нельзя работать со структурами напрямую, так как их не сгенерируешь; например, платформа принимает из массива только массив байт, отчего приходится кастить из разных типов
- также при написании тестов, необходимо было перенастроить сам способ написания, с обработки конкретных случаев к обработке ожидаемых результатов
- время ожидания - из этого пункта исходит то, что разработка и выкатка нового функционала может быть задержана

Ошибки, которые были выявлены:
- при отсутствии кастинга получаются неверные резульаты в виду разницы в типах данных

На удивление, тесты не показали ошибок в программе, что несомненно радует!

Также при изучении fuzzing тестирования узнал, что такое инварианты - условия, которые всегда должны быть истинными, при вызове любого публичного метода. И оракулы - условия, которые мы ожидаем при том или ином изменении в данном контексте.

Итогом для меня стало следующее:
- fuzzing тестирование - вид тестирования, который помогает эффективно обнаруживать проблемы в программе; написания тестов заключается в создании примеров, описании инвариантов/оракулов и их проверок

Буду пробовать применять данный вид тестирования в будущих проектах, так как он мне кажется более эффективным, в плане написания тестов и обнаружения проблем в программе.